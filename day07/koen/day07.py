...
def add_joker_rank(df: pl.DataFrame) -> pl.DataFrame:
    option_counts = [pl.col('hand').str.count_matches(x, literal=True).alias(f'o_{x}') for x in OPTIONS]
    count_columns = [f'o_{x}' for x in OPTIONS if x != 'J']
    df = (
        df
        .with_columns(
            pl.col('hand')
            ...
            .str.replace_all('K', 'd', literal=True)
            .str.replace_all('Q', 'c', literal=True)
            .str.replace_all('J', '1', literal=True)
            .str.replace_all('T', 'a', literal=True)
            .str.parse_int(16)
            .alias('hand_hex'),
            *option_counts
        ...
        .with_columns(
            pl.when((pl.col('count1') + pl.col('o_J')).eq(5))
            .then(pl.lit(7))
            .when((pl.col('count1') + pl.col('o_J')).eq(4))
            .then(pl.lit(6))
            .when((pl.col('count1') + pl.col('o_J')).eq(3) & pl.col('count2').eq(2))
            .then(pl.lit(5))
            .when((pl.col('count1') + pl.col('o_J')).eq(3))
            .then(pl.lit(4))
            .when(pl.col('count1').eq(2) & pl.col('count2').eq(2))
            .then(pl.lit(3))
            .when((pl.col('count1') + pl.col('o_J')).eq(2))
            .then(pl.lit(2))
            .otherwise(pl.lit(1))
            .alias('prio')
        )
